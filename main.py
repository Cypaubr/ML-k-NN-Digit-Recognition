...
best_score = errors.index(np.amin(errors)) + 2
worse_score = errors.index(np.amax(errors)) + 2
print('Best score %f' % best_score)
print('Worse score %f' % worse_score)
...
print('Predicting using worse score...')
knn = neighbors.KNeighborsClassifier(worse_score)
knn.fit(xtrain, ytrain)
predicted = knn.predict(xtest)
images = xtest.reshape((-1, 28, 28))
select = np.random.randint(images.shape[0], size=12)
